...
//   202507969 Maria Brebenel
//   202505552 Nikola Karazlatev
// Contributions:
...
public class Inversions {
    public int countInversions(ArrayList<Integer> input) {
        ...
        return mergeSort(input.toArray(new Integer[input.size()]));
    }
    public int mergeSort(Integer[] array) {
        if (array.length <= 1) {
            return 0;
        }
        ...
        return inversions + merge(array, left, right);
    }

    private int merge(Integer[] array, Integer[] left, Integer[] right) {
        int i = 0, j = 0, k = 0;
        int inversions = 0;
